...
const entryProcessors = {
  section: (section, maxVersion, depth = 1) =>
    [`${'#'.repeat(depth)} ${section.name}`]
      .concat(
        section.content.map(e => print(e, maxVersion, depth + 1))
      )
      .join('\n\n'),
  content(entry, maxVersion) {
    const def = entry.value
    if (def.kind === 'interface') {
      const legalBases = def.bases.filter(base => !base.added || base.added <= maxVersion).map(base => base.added ? base.name : base)
      ...
      return '```js\ninterface ' + def.name + bases + ' ' + typeProcessors.object({ items: def.props }, maxVersion) + '\n```' + (doc ? '\n\n' + doc : '')
    } else if (def.kind === 'enum') {
      return '```js\nenum ' + def.name + ' {\n    ' + def.values.filter(v => !v.added || v.added <= maxVersion).map(typeProcessors.literal).join(' | ') + '\n}\n```' + (def.doc ? '\n\n' + def.doc : '')
    } else {
  ...
  object: ({items}, maxVersion) => {
    ...
    indent(() => {
      for (const prop of allowed) {
        ...
        result += indentation + `${prop.name}: ${processType(prop.type, maxVersion)};\n`;
      }
    }, ' '.repeat(4));
    result += indentation + '}';
    return result;
...
export default function toMarkdown(spec, maxVersion = Infinity) {
  ...
  for (const def of spec) {
    ...
    const sectionName = def.section ? def.section : def.bases[0] ? [def.bases[0]] : ['<root>']
